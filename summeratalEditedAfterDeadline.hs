...
main = do
    ...
    line2 <- getLine
    let listOfIntAsStrings  =  words $ line2
        lenghtOfList = length listOfIntAsStrings
        x =  sum $ map (read :: String-> Int) $ drop (lenghtOfList `div` 2) $ sort $ listOfIntAsStrings
        in  putStrLn $ show $ x
    --putStrLn $ "mapexsum :" ++ show(sum . map (read :: String-> Int)  $ drop (div (length ) 2)  $ words $ line2)
    --print show(take 1 . words  line)
